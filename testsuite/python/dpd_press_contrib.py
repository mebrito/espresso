...
class NoDPDPressContrib(ut.TestCase):
    ...
    def test_nondpd_case(self):
        ...
        self.assertNotIn("dpd", energy_dict,
                         "'dpd' key should not be in the energy dictionary")

        pressure_dict = self.system.analysis.pressure()
        self.assertNotIn("dpd", pressure_dict,
                         "'dpd' key should not be in the pressure dictionary")

        pressure_tensor_dict = self.system.analysis.pressure_tensor()
        self.assertNotIn("dpd", pressure_tensor_dict,
                         "'dpd' key should not be in the pressure-tensor dictionary")
...
class DPDPressContrib(ut.TestCase):
    ...
    def test_dpd_case(self):
        ...
        # Add particles
        self.system.part.add(pos=self.system.box_l *
        ...
        self.system.non_bonded_inter[0, 0].hat.set_params(
            F_max=F_max, cutoff=r_cut)
        ...
        pressure_dict = self.system.analysis.pressure()
        self.assertIn("dpd", pressure_dict,
                      "'dpd' key is not in the pressure dictionary")

        pressure_tensor_dict = self.system.analysis.pressure_tensor()
        self.assertIn("dpd", pressure_tensor_dict,
                      "'dpd' key is not in the pressure-tensor dictionary")
